sql_packingHouse_query = '''
               with summaryPackingHouseData as (
                ...
                If(Grouping(packingHouseName),'', packingHouseName) as  packingHouseName, 
                ...
                Group By  fruitName, fruitType, packingHouseName with rollup

                )

                , summaryPackingHouseDataPivotFruit as(
                select 
                fruitName,  
                packingHouseName, 
                FORMAT(round(sum(weightNeto),0),0) as weightNeto,
                ...
                FORMAT(round(sum(payment),0),0) as payment
                from summaryPackingHouseData
                group by fruitName, packingHouseName 
                order by packingHouseName
                ...
                total_weight_to_pay,
                payment
                from summaryPackingHouseDataPivotFruit
